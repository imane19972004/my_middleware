    ...
    public class JCDService : IJCDService
    {
        ...
        public async Task<Station> GetClosestStationAsync(SimplifiedGeoCoordinate coordinates, string city, int minBikes)
        {
            ...
            if (stations == null || stations.Count == 0)
                stations = await ForceGetAvailableStationsAsync(minBikes);

            var closestStation = stations[0];
            var closestDistance = geoCoordinates.GetDistanceTo(
                new GeoCoordinate(closestStation.position.lat, closestStation.position.lng));

            foreach (var station in stations)
            {
                var distance = geoCoordinates.GetDistanceTo(
                    new GeoCoordinate(station.position.lat, station.position.lng));
                ...
                var cachedResponse = await _cachingServer.GetOrSet(cacheKey,
                    async () =>
                    {
                        var response = await _client.GetStringAsync(
                            $"{Constants.JcdBaseAddress}?apiKey={apiKey}&contract={city}");
                        return response;
        ...
        private async Task<List<Station>> GetStationsAsync()
        {
            ...
            var cachedResponse = await _cachingServer.GetOrSet(cacheKey,
                ...
                {
                    var response = await _client.GetStringAsync(
                        $"{Constants.JcdBaseAddress}?apiKey={apiKey}");
                    return response;
                }, TimeSpan.FromMinutes(5));
