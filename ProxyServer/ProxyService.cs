    ...
    public class JCDService : IJCDService
    {
        ...
        public JCDService()
        {
            ...
            if (string.IsNullOrEmpty(_apiKey))
            {
                ...
                _apiKey = "753fd31b1a7a1b3a69f17aeaa2ca112f575bf9d6"; // Votre clé du .env.txt
            }

            Console.WriteLine($"✅ JCDService initialisé avec clé: {_apiKey.Substring(0, 10)}...");
        }
            ...
            try
            {
                var result = await _cache.GetOrSet(
                    ...
                    async () =>
                    {
                        var url = $"{Constants.JcdBaseAddress}?contract={contractName}&apiKey={_apiKey}";
                        Console.WriteLine($"🌐 API JCDecaux: {contractName}");
                        Console.WriteLine($"   URL: {url}"); // ✅ DEBUG
                        ...
                        {
                            // ✅ Si Nice échoue, essayer Lyon comme fallback
                            if (contractName.Equals("Nice", StringComparison.OrdinalIgnoreCase))
                            {
                                Console.WriteLine($"⚠️ Contrat Nice non disponible, tentative avec Lyon...");
                                var fallbackUrl = $"{Constants.JcdBaseAddress}?contract=Lyon&apiKey={_apiKey}";
        ...
        public async Task<Station> GetClosestStationAsync(SimplifiedGeoCoordinate coordinates, string city, int minBikes)
        {
            try
            {
                Console.WriteLine($"🔍 GetClosestStationAsync appelé:");
                Console.WriteLine($"   Coordinates: {coordinates?.Latitude}, {coordinates?.Longitude}");
                Console.WriteLine($"   City: {city}");
                Console.WriteLine($"   MinBikes: {minBikes}");
                ...
                if (string.IsNullOrEmpty(jsonStations))
                {
                    Console.WriteLine($"❌ Pas de JSON reçu pour {city}");
                    return null;
                }
                ...
                Console.WriteLine($"✅ {stations.Length} stations récupérées pour {city}");
                ...
                Console.WriteLine($"   {availableStations.Count} stations avec {minBikes}+ vélos");
            ...
            catch (Exception ex)
            {
                ...
                throw new FaultException($"Erreur: {ex.Message}");
            }
        }
