    ...
    public class RestController
    {
        ...
        private async Task HandleItineraryRequest(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            using (var reader = new StreamReader(request.InputStream, Encoding.UTF8))
            {
                ...
                // ✅ DEBUG: Afficher le JSON brut reçu
                Console.WriteLine($"\n🔍 JSON brut reçu: {body}");
        ...
        private async Task HandleAutocompleteRequest(HttpListenerRequest request, HttpListenerResponse response)
        {
            // ✅ DÉCODER CORRECTEMENT LES PARAMÈTRES URL
            ...
            if (!string.IsNullOrEmpty(queryString))
            {
                ...
                // Parser manuellement pour gérer UTF-8
                var parts = queryString.Split('=');
                if (parts.Length == 2 && parts[0] == "q")
                {
                    query = System.Web.HttpUtility.UrlDecode(parts[1], Encoding.UTF8);
                }
            }

            Console.WriteLine($"🔍 Autocomplete UTF-8: '{query}'");
